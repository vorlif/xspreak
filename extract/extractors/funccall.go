...
func NewFuncCallExtractor() extract.Extractor { return &funcCallExtractor{} }

func (v funcCallExtractor) Run(_ context.Context, extractCtx *extract.Context) ([]extract.Issue, error) {
	...
	extractCtx.Inspector.Nodes([]ast.Node{&ast.CallExpr{}}, func(rawNode ast.Node, push bool) (proceed bool) {
		...
		if ident == nil {
			if selector := util.SearchSelector(node.Fun); selector != nil {
				ident = selector.Sel
			} else {
				return
			}
